...
function send_mail($email, $name, $token, $type)
{
    ...
    try {
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'jalen.1720@gmail.com';
        $mail->Password = 'vbyaqzlxbuvwdqjq';  // Make sure to handle credentials securely
        $mail->SMTPSecure = 'ssl';
        $mail->Port = 465;

        //Recipients
        $mail->setFrom('jalen.1720@gmail.com', 'No-Reply');
        ...
        $mail->Body = "Click the link below to $content: <br>
        <a href='http://localhost/tokyowebsite/$page?$type&email=$email&token=$token'>Click Me</a>";
...
if (isset($_POST['forgot_pass'])) {
    ...
    if (mysqli_num_rows($u_exist) == 0) {
        ...
        if ($u_fetch['is_verified'] == 0) {
            ...
            // You should implement the send_mail() function
            if (!send_mail($data['email'], $u_fetch['name'], $token, 'account_recovery')) {
                ...
                $query_result = update($query, $values, 'ssi');
